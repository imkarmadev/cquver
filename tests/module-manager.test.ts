...
Deno.test('ModuleManagerService - generates correct type index content', async () => {
    ...
    try {
        ...
        await Deno.writeTextFile(`${testPath}/create-user/create-user.handler.ts`, `
export class CreateUserCommandHandler {
  async execute() {}
}
    `);

        await Deno.writeTextFile(`${testPath}/update-user/update-user.handler.ts`, `
...
Deno.test('ModuleManagerService - creates new module file when none exists', async () => {
    ...
    try {
        ...
        const moduleContent = await Deno.readTextFile(`apps/src/test-service/src/test-service.module.ts`);
