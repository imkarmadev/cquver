...
Deno.test('eventHandlerTemplate - generates correct event handler', () => {
    const result = eventHandlerTemplate('UserCreatedEvent', 'UserCreatedEventHandler', 'user-created-event');
    ...
    assertStringIncludes(result, 'export class UserCreatedEventHandler implements IEventHandler<UserCreatedEvent>');
...
Deno.test('eventIndexTemplate - generates correct event index exports', () => {
    const result = eventIndexTemplate('UserCreatedEvent', 'UserCreatedEventHandler', 'user-created-event');

    assertStringIncludes(result, "export { UserCreatedEvent } from './user-created-event.event';");
    assertStringIncludes(result, "export { UserCreatedEventHandler } from './user-created-event.handler';");
});
...
Deno.test('commandHandlerTemplate - generates correct command handler', () => {
    const result = commandHandlerTemplate('CreateUserCommand', 'CreateUserCommandHandler', 'create-user-command');

    assertStringIncludes(result, "import { CommandHandler, ICommandHandler } from '@nestjs/cqrs';");
    assertStringIncludes(result, "import { CreateUserCommand } from './create-user-command.command';");
    assertStringIncludes(result, '@CommandHandler(CreateUserCommand)');
    assertStringIncludes(result, 'export class CreateUserCommandHandler implements ICommandHandler<CreateUserCommand>');
...
Deno.test('queryHandlerTemplate - generates correct query handler', () => {
    ...
    assertStringIncludes(result, 'export class GetOrderQueryHandler implements IQueryHandler<GetOrderQuery>');
