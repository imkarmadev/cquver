...
Deno.test('CLI - successfully creates event', async () => {
    ...
    try {
        ...
        assertEquals(result.code, 0);
        assert(result.stdout.includes('Successfully generated event "UserCreated" for app "test-service"'));
        assert(result.stdout.includes('Created directory'));
        assert(result.stdout.includes('Created file'));

        // Verify files were created
        assert(await exists('apps/src/test-service/application/events/user-created/user-created.event.ts'));
        assert(await exists('apps/src/test-service/application/events/user-created/user-created.handler.ts'));
...
Deno.test('CLI - successfully creates command', async () => {
    ...
    try {
        ...
        assertEquals(result.code, 0);
        assert(result.stdout.includes('Successfully generated command "CreateUser" for app "auth-service"'));

        // Verify files were created
        assert(await exists('apps/src/auth-service/application/commands/create-user/create-user.command.ts'));
        assert(await exists('apps/src/auth-service/application/commands/create-user/create-user.handler.ts'));
...
Deno.test('CLI - successfully creates query', async () => {
    ...
    try {
        ...
        assertEquals(result.code, 0);
        assert(result.stdout.includes('Successfully generated query "GetOrdersByUser" for app "order-service"'));

        // Verify files were created with correct pluralization
        assert(await exists('apps/src/order-service/application/queries/get-orders-by-user/get-orders-by-user.query.ts'));
        assert(await exists('apps/src/order-service/application/queries/get-orders-by-user/get-orders-by-user.handler.ts'));
...
Deno.test('CLI - handles complex naming correctly', async () => {
    ...
    try {
        ...
        const eventFile = await Deno.readTextFile('apps/src/test-service/application/events/xml-http-request-completed/xml-http-request-completed.event.ts');
        assert(eventFile.includes('export class XMLHttpRequestCompletedEvent'));

        const commandFile = await Deno.readTextFile('apps/src/test-service/application/commands/update-user-profile/update-user-profile.command.ts');
