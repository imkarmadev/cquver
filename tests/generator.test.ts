...
Deno.test('GeneratorService - creates event with correct structure', async () => {
    ...
    try {
        ...
        assert(await exists(`apps/src/${appName}/application/events/user-created/user-created.event.ts`));
        assert(await exists(`apps/src/${appName}/application/events/user-created/user-created.handler.ts`));
        ...
        // Check file contents
        const eventFile = await Deno.readTextFile(`apps/src/${appName}/application/events/user-created/user-created.event.ts`);
        assert(eventFile.includes('export class UserCreatedEvent implements IEvent'));

        const handlerFile = await Deno.readTextFile(`apps/src/${appName}/application/events/user-created/user-created.handler.ts`);
...
Deno.test('GeneratorService - creates command with correct structure', async () => {
    ...
    try {
        ...
        assert(await exists(`apps/src/${appName}/application/commands/create-user/create-user.command.ts`));
        assert(await exists(`apps/src/${appName}/application/commands/create-user/create-user.handler.ts`));
        assert(await exists(`apps/src/${appName}/application/commands/index.ts`));

        // Check file contents
        const commandFile = await Deno.readTextFile(`apps/src/${appName}/application/commands/create-user/create-user.command.ts`);
        assert(commandFile.includes('export class CreateUserCommand implements ICommand'));

        const commandsIndex = await Deno.readTextFile(`apps/src/${appName}/application/commands/index.ts`);
...
Deno.test('GeneratorService - creates query with correct structure', async () => {
    ...
    try {
        ...
        // Check file contents
        const queryFile = await Deno.readTextFile(`apps/src/${appName}/application/queries/get-order/get-order.query.ts`);
        assert(queryFile.includes('export class GetOrderQuery implements IQuery'));

        const queriesIndex = await Deno.readTextFile(`apps/src/${appName}/application/queries/index.ts`);
