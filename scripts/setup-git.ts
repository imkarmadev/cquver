...
async function showSummary(): Promise<void> {
  ...
  console.log(colorize('  📝 Interactive commit: ', colors.yellow) + colorize('deno task commit', colors.blue));
  console.log(colorize('  📝 or use git alias: ', colors.yellow) + colorize('git cc', colors.blue));
  console.log(colorize('  📝 Regular commit: ', colors.yellow) + colorize('git commit', colors.blue) + colorize(' (will show template)', colors.yellow));
  console.log(colorize('  📝 Changelog: ', colors.yellow) + colorize('deno task changelog v1.0.0', colors.blue));
  
  console.log(colorize('\n💡 VSCode Extensions (recommended):', colors.cyan));
  console.log(colorize('  - Conventional Commits (vivaxy.vscode-conventional-commits)', colors.yellow));
  ...
  console.log(colorize('  - docs/RELEASE_WORKFLOW.md - Release workflow documentation', colors.yellow));
}

async function main(): Promise<void> {
  console.log(colorize('🛠️  Git Conventional Commits Setup', colors.bright + colors.cyan));
  console.log(colorize('Setting up git configuration for conventional commits and automated workflows.\n', colors.cyan));

  // Check if we're in a git repository
  if (!(await checkGitRepo())) {
    console.log(colorize('❌ Not in a git repository. Please run this from the project root.', colors.red));
    Deno.exit(1);
  }
  ...
  if (allSuccess) {
    ...
    console.log(colorize('\n⚠️  Setup completed with some warnings. Check the messages above.', colors.yellow));
  }
}
