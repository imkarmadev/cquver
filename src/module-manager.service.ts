 ...
 */
    ...
    async updateServiceModule(appName: string): Promise<void> {
        ...
        try {
            ...
            console.warn(`⚠️ Could not update module file: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
     ...
     */
    private async getAllHandlers(typePath: string, type: 'command' | 'event' | 'query'): Promise<HandlerInfo[]> {
 ...
 */
    private generateTypeIndexContent(type: 'command' | 'event' | 'query', handlers: HandlerInfo[]): string {
    ...
    private generateNewModuleContent(appName: string): string {
        const className = appName.split('-').map(word =>
